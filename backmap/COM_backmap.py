...
def COM_backmap(CG_struct, AA_trj, CG_bead, AA_bead):
    ...
    CG_mol_inds = np.asarray([np.sort([atom.index for atom in mol]) for mol in [list(mol) for mol in CG_struct.top.find_molecules()]])
    AA_mol_inds = np.asarray([np.sort([atom.index for atom in mol]) for mol in [list(mol) for mol in AA_new_trj.top.find_molecules()]])
